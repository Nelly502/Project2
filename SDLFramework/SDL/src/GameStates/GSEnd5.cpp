...
void GSEnd5::Init()
{
	...
	texture = ResourceManagers::GetInstance()->GetTexture("bg_pick.tga");
	m_background3 = std::make_shared<Sprite2D>(texture, SDL_FLIP_NONE);
	m_background3->SetSize(SCREEN_WIDTH, SCREEN_HEIDHT);
	m_background3->Set2DPosition(0, 0);
	m_background3->setActive(false);
	...
	//m_textGameName1->Set2DPosition(-300,-200 );
	m_textGameName1->LoadFromRenderText("VICTORY");

	m_textColor1 = { 255, 255, 0 };
	m_textTiso1 = std::make_shared<Text>("Data/Number.ttf", m_textColor1);
	m_textTiso1->SetSize(150, 150);
	m_textTiso1->Set2DPosition(320,200 );
	m_textTiso1->LoadFromRenderText(std::to_string(ResourceManagers::GetInstance()->demWin1));

	m_textColor1 = { 255, 255, 0 };
	m_textTiso2 = std::make_shared<Text>("Data/Number.ttf", m_textColor1);
	m_textTiso2->SetSize(150, 150);
	m_textTiso2->Set2DPosition(SCREEN_WIDTH - 420, 200);
	m_textTiso2->LoadFromRenderText(std::to_string(ResourceManagers::GetInstance()->demWin2));

	m_textColor2 = { 0, 0, 0 };
	m_textGameName2 = std::make_shared<Text>("Data/HalloweenGuises.ttf", m_textColor2);
	m_textGameName2->SetSize(300, 200);
	//m_textGameName2->Set2DPosition(-300, -200);
	m_textGameName2->LoadFromRenderText("VICTORY");
	// button close
	...
	buttonCon->SetOnClick([this]() {
		GameStateMachine::GetInstance()->ChangeState(StateType::STATE_FINISH);
		});
...
void GSEnd5::HandleEvents()
{
}

void GSEnd5::HandleKeyEvents(SDL_Event& e)
{

}

void GSEnd5::HandleTouchEvents(SDL_Event& e)
{
...
void GSEnd5::HandleMouseMoveEvents(int x, int y)
{
}

void GSEnd5::Update(float deltaTime)
{
	...
	/*if (ResourceManagers::GetInstance()->demWin1 = ResourceManagers::GetInstance()->demWin2) {
		...
		m_textGameName1->Set2DPosition(600, 45);
		m_background3->setActive(true);
	}*/
...
void GSEnd5::Draw(SDL_Renderer* renderer)
{
	...
	/*if (m_background3->getActive()) {
		m_background3->Draw(renderer);
	}*/
	...
	m_textTiso2->Draw(renderer);
	m_textTiso1->Draw(renderer);
